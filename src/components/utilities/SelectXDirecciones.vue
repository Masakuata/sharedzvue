<template>
    <div :class="[{ 'relative': isOpen  }]">
        ...
        <div @click="isOpen = !isOpen" class="w-full rounded-b-lg bg-bgBlue p-1 flex flex-row  text-white font-semibold text-center items-center">

            <template v-if="elementoSeleccionado">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                    stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                    class="lucide lucide-map-pinned w-2/12 text-white my-1">
                    <path d="M18 8c0 4.5-6 9-6 9s-6-4.5-6-9a6 6 0 0 1 12 0" />
                    <circle cx="12" cy="8" r="2" />
                    <path
                        d="M8.835 14H5a1 1 0 0 0-.9.7l-2 6c-.1.1-.1.2-.1.3 0 .6.4 1 1 1h18c.6 0 1-.4 1-1 0-.1 0-.2-.1-.3l-2-6a1 1 0 0 0-.9-.7h-3.835" />
                </svg>
                <p class="w-11/12 text-left truncate">{{ elementoSeleccionado.direccion }}</p>
            </template>
        ...
        <div v-show="isOpen" class="absolute  w-full bg-gray-200 shadow-xl rounded max-h-40 overflow-scroll "
            ...
            <div class="flex flex-col">
                ...
                <div v-for="elemento in props.elementos" :key="elemento.id"
                    class="p-1  cursor-pointer rounded-lg mt-1" @click="selectItem(elemento)">
                    {{ elemento.direccion }}
                </div>
            </div>
        </div>
